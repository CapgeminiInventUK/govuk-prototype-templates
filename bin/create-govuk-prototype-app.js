...
const configureHmrcFrontend = require("./scripts/hmrc");
    ...
    if (answers.action === "Create a prototype from scratch") {
      inquirer
        ...
        .then(async ({ projectName, templates }) => {
          ...
          checkProjectExists(projectName, projectPath);
          ...
          try {
            ...
            if (templates.includes("HMRC Frontend")) {
              await configureHmrcFrontend(projectPath);
            }
      ...
      inquirer
        ...
        .then(async (answers) => {
          ...
          if (answers.templates.includes("HMRC Frontend")) {
            const projectPath = process.cwd();
            configureHmrcFrontend(projectPath);
          }
        });
    }
  });
